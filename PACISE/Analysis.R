...
# Analysis of Datasets with Tree-Based Methods Using 70-30 Training-Test Split
...
Cleve.rf <- randomForest(target ~ ., data = Cleve.train, mtry = 4, ntree = 50,
                         importance = TRUE)
...
Cleve.boost <- gbm(target ~ ., data = Cleve.train, distribution = "bernoulli",
                   n.trees = 5000, interaction.depth = 7, shrinkage = 0.1)
boost.probs <- predict(Cleve.boost, newdata = Cleve.test, n.trees = 5000,
                       type = "response")
boost.preds <- rep(0, 307)
...
# *** The code below is retained from original script in case the Fram 
# *** dataset will also be tested at the 70–30 split.
# ==================
# Framingham Dataset
# ==================
# Import dataset and prepare it for analysis
